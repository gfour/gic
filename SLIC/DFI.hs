...
import qualified Data.Map as M (lookup, toList, unions)
     ...
     (case M.lookup mn builtinModules of
         ...
         Nothing ->
           do (case filter (\d->(fst.dfiMInfo) d==mn) dfis of
              ...
              let import' = IDecl mn (fromList mn') (Just (fSigs, cids))
              return (import' : rest))
...
calcExtDFInfo (Prog [Data _ _ dcs] defs) =
  let clcs           = fromList $ map 
                       (\(DConstr c dts _) -> (c, length dts)) dcs
      isNotCC (f, _) = not $ member f clcs 
      apps = fromList $ filter isNotCC $ map (\(DefF f s _) -> (f, length s)) defs
  in  (clcs, apps)
