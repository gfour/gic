...
import SLIC.DFI (DfConstrs, DfInfo(..), DFI(..), DfInfo(DfInfo), DFC(DFC),
                 ExtAppFuns, LARInfo(LARInfo, liCIDs, liPMDs), addApp,
                 emptyDfInfo, mergeDfInfo, noApps)
import SLIC.Front.Preprocessor (constrToFuncs)
import SLIC.State
...
defuncMod opts ve' m'@(Mod fm@(mN, _) exportsR importsR (Prog dts defs) an tcs) =
  let vFuns     = visibleFuns ve' m'
      ...
      (dfDefs, dfI) = unzipI $ List.map (defuncD ve' vFuns) defs
      dfcs = diDfcs dfI
      ...
      dfClosDT = mkClosureGADT opts dfcs
...
defuncEL ve vfs lvs el =
  let (el', infoL) = unzip $ List.map (defuncE ve vfs lvs) el
  in  (el', mergeDfInfo infoL)

-- | Defunctionalize an expression.
defuncE :: TEnv -> VFuns -> LVars -> ExprF -> DfState ExprF
defuncE _ vfs _ e@(XF (V vName)) =
...
defuncE _ _ _ bv@(XF (BV _ _)) = (bv, emptyDfInfo)
defuncE ve vfs lvs (ConF b el) =
  let (el', dfInfo) = defuncEL ve vfs lvs el
  in  (ConF b el', dfInfo)
...
defuncE _ _ _ (ConstrF _ _) =
  ierr "defuncE: non-top-level constructor encountered"
defuncE ve vfs lvs (FF f@(V fName) el) =
  let (el', dfInfoL)  = defuncEL ve vfs lvs el
            ...
            Just (fT, Just fArity)  ->       ----- top-level function -------
              if fArity == argsLen then      ----- saturated application ----
                (FF f el', dfInfoL)
                     ...
                     pappToConstr fName fT fArity argsLen el' dfInfoL
                   else
...
--   threaded.
pappToConstr :: QName -> Type -> Arity -> Int -> [ExprF] -> DfInfo ->
                DfState ExprF
pappToConstr fName fT fArity argsLen el' dfInfo =
...
-- 
genDfCode :: Options -> DfConstrs -> ExtAppFuns -> ProgF
genDfCode opts dfcs extApps =
  Prog [mkClosureGADT opts dfcs] (mkApplyFuns opts dfcs extApps)
...
--   declarations.
genDfMod :: Options -> DFI -> ModF
genDfMod opts (DFI _ env fsigs (DfInfo dfcs extApps) larInfo _) =
  let dfCode@(Prog _ appDefs) = genDfCode opts dfcs extApps
...
genDfModFinal opts dfi =
  let dfModF = constrToFuncs opts $ genDfMod opts dfi
...
mkApplyFuns :: Options -> DfConstrs -> ExtAppFuns -> [DefF]
mkApplyFuns opts dfcs extApps =
  let usedArities = clArities dfcs
      -- the parameters given to the closure
      nParams ar = List.map (genCla ar) [0..(ar-1)]
      appBody ar params pats =
        let -- the function formals
            frmNames = (genCl ar):params
            strictness = optStrict opts
      ...
      aux ar =
        let params = nParams ar
            ...
            appFull = mkApply mkPatFull dfcsF params
            ...
            appPartial = mkApply mkPatPartial dfcsP params
...
--   defunctionalization.
mkClosureGADT :: Options -> DfConstrs -> Data
mkClosureGADT opts dfcs =
  let mkGCConstr (DFC c ar _ (_, t)) =
        let ts = types t
            comps = List.map (\t0->DT t0 defaultStrictness Nothing) $ take ar ts
            retT  =
              case drop ar ts of
