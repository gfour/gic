...
import qualified Data.Set as S (empty, filter, fromList, map, toList, union)
...
type DfFlags = (DoNullDf, Strictness, ScrutOpt, TypeChecker, Sharing)
...
unzipI aInfos =
  (map fst aInfos, mergeDfInfo (map snd aInfos))
...
defuncMod opts ve' m'@(Mod fm@(mN, _) exportsR importsR (Prog dts defs) an tcs) =
  let vFuns     = visibleFuns ve' m'
      ...
      flags@(ndf, _, _, _, _) = dfFlags opts
      ...
      (dfDefs, dfI) = unzipI $ map (defuncD ndf ve' vFuns) defs
      dfcs = diDfcs dfI
      -- 3. Replace higher-order components in data types with closure data types.
      dfDataC (DConstr c ts rt) =
        DConstr c (map (\(DT t s sel)->DT (lowT t) s sel) ts) (fmap lowT rt)
      dfData    = map (\(Data dt as cs) ->
                             Data dt as (map dfDataC cs)) dts
      ...
      prettyPrinters = map (\(Data dt _ _)->QN (Just mN) (pprint dt "")) dts
      ...
      tcs' = map defuncTcDecl $ tcIDecls tcs
...
dfImports dfEnv dfClosDT dfSigs = 
  let mkIInfoApp (f, fs) =
      ...
      mkIInfoCC (Data _ _ dcs) = 
        ...
        in  map aux dcs      
      iDfNames = M.fromList ((mkIInfoCC dfClosDT)++(map mkIInfoApp $ M.toList dfSigs))
...
defuncTcDecl (TcDecl tcn tv methods) =
  TcDecl tcn tv (map (\(sig, t) -> (sig, defuncT t)) methods)
...
visibleFuns ve modF = 
  let imports = modImports modF
      ...
      localFuns :: TEnv
      localFuns   = M.fromList $ map (\(DefF f fs _)->(f, (findType f ve, Just (length fs)))) defs
      ...
      vfsImported = M.fromList $ mapMaybe iEnvToDfEnv 
                    (concatMap M.toList $ map ideclINames imports)
...
makeAppInfo ve dfcs =
  let dfcTs :: TEnv
      dfcTs = M.fromList $ map typeDFC $ S.toList dfcs
      appTs = concatMap (appDFC ve) $ S.toList dfcs
      -- the types of closure dispatchers
      appTsVE :: TEnv
      appTsVE   = M.fromList $ map (\(f, _, ei)->(f, ei)) appTs
      ...
      appFSigs  = M.fromList $ map (\(f,fs,_)->(f, map fst fs)) appTs 
      -- the signatures of closure constructors
      clSigs = M.fromList
               (map (\(DFC c a _ _)->(c, cArgsC c a)) $ S.toList dfcs)
...
appDFC ve (DFC _ ar rT (f, _)) =
  case M.lookup f ve of
    Just (t, Just fullAppArity) ->
      ...
      in  (map appPartT [1..argsLeft-1])++[appFullT argsLeft]
    Just (_, Nothing) ->
...
pprintAppInfo apps =
  pprFSigs $ M.fromList $ map (\(qn, params, _)->(qn, map fst params)) apps
-}
...
genAllAppSigs ve defs =
  let genAppSigsD lvs (DefF _ frms e) = genAppSigsE ((frmsToNames frms)++lvs) e
      ...
      genAppSigsE lvs (CaseF _ e _ pats) =
        unions $ (genAppSigsE lvs e) : (map (genAppSigsP lvs) pats)
      genAppSigsE lvs (LetF _ ldefs e) =
        unions $ (genAppSigsE lvs e) : (map (genAppSigsD lvs) ldefs)
              ...
              case M.lookup fName ve of
                ...
                Just _ ->
                  let appName = genNApp argsLen
                      appFrms = (genCl argsLen) : (map (genCla argsLen) [0..(argsLen-1)])
                      argsLen = length el
                  in  M.union (M.fromList [(appName, appFrms)]) (genAppSigsL lvs el)
            else
              genAppSigsL lvs el
      genAppSigsP lvs (PatB (SPat _ bvs, _) e) = genAppSigsE (bvs++lvs) e
      genAppSigsL lvs el = unions $ map (genAppSigsE lvs) el
  in  unions $ map (genAppSigsD []) defs
...
defuncEL ndf ve vfs lvs el =
  let (el', infoL) = unzip $ map (defuncE ndf ve vfs lvs) el
...
defuncE ndf ve vfs lvs (CaseF d e v pats) =
  let defuncPat (PatB sPat@(SPat c bvs, _) eP) =
      ...
      (pats', (DfInfo dfcsL appsL)) = unzipI $ map defuncPat pats
  in  (CaseF d e' v pats', DfInfo (S.union dfcsE dfcsL) (S.union appsE appsL))
...
allClosuresFor v t arities =
  S.fromList $ map (\j->(DFC (genNC v j) j (residualT j t) (v, t))) arities
...
defuncT :: Type -> Type
defuncT typ = constrT (map lowT $ takeParams typ) (lowT $ last $ types typ)
...
dfiAppSigs dfi =
  let DfInfo _ dfAppArs = dfiDfInfo dfi
      appSig ar = (genNApp ar, (genCl ar):(map (genCla ar) [0..(ar-1)]))
...
genDfMod flags (DFI _ env fsigs (DfInfo dfcs extApps) larInfo _) =
  let dfCode@(Prog _ appDefs) = genDfCode flags dfcs extApps
      dfExps = M.fromList $ map defSig appDefs
      ...
      -- all imports
      dfImps = map iMod (S.toList iModNames)
...
genDfInfo (_, str, scr, _, sha) defs =
  let defSigs = M.fromList $ map defSig defs
      appCBNs = genAppCBNs sha scr defs
...
genDfModFinal flags@(_, _, scr, tc, _) dfi =
...
genAppCBNs :: Sharing -> ScrutOpt -> [DefF] -> CBNVars
genAppCBNs sharing scrOpt defs =
  let cbnDef (DefF f _ (ConstrF _ _)) = (f, [])
      cbnDef (DefF f fs@((Frm cl _):args) (CaseF _ (XF (V cl')) _ _)) | cl == cl' =
        if not sharing then (f, [])
        else if scrOpt then (f, frmsToNames args)
             else (f, frmsToNames fs)
      cbnDef def@(DefF {}) =
        ierr $ "cbnDef: strange defunctionalization code: "++(pprint def "")
  in  M.fromList $ map cbnDef defs
...
genAppStricts str defs =
  ...
  in  M.fromList $ map onlyIfStrict defs
...
mkApplyFuns (ndf, strictness, scrOpt, _, _) dfcs extApps =
  let -- If doing nullary defunctionalization, keep only 0-arg closures.
      ...
      aux ar =
        ...
        in  if size (S.union dfcsF dfcsP) == 0 then
      ...
      missingAppDefs = map (\ar -> appBody ar (nParams ar) []) missingApps
...
mkApply scrOpt mkPat dfcs frms = 
  let frmsLen  = length frms
      app      = genNApp frmsLen
  in  map (mkPat scrOpt app frms) (S.toList dfcs)
...
mkPatFull scrOpt app frms (DFC c ar _ (f, _)) =
  let bns = cArgsC c ar
      bvs = map (\v->XF $ BV v (cNested scrOpt 0 app)) bns
      pI  = PatInfo (ar/=0)
  in  PatB (SPat c bns, pI) (FF (V f) (bvs ++ (map (\v->XF $ V v) frms)) NoCI)
...
mkPatPartial scrOpt app frms (DFC c ar _ (f, _)) =
  ...
  in  PatB (SPat c bns, pI) $
      ConstrF (genNC f (ar + length frms)) (bvs ++ (map (\v->XF $ V v) frms))
...
mkClosureGADT (ndf, s, _, _, _) dfcs =
  let mkGCConstr dfc@(DFC _ ar _ _) | ndf && ar>0 =
      ...
      allConstrs = map mkGCConstr $ S.toList dfcs
