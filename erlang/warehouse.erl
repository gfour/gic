...
spawn_warehouses(N, MAX_SZ_CTXTS) ->
	...
	0 -> [];
	_ ->
	    WPID = spawn(warehouse, winit, [N, MAX_SZ_CTXTS]),
...
wrun(ID, Ctxts, CID, WValues, PendingSlots) ->
	...
	{demand, PID, V, C} ->
		...
		error ->
		    ...
		    PendingSlots2 = dict:append(PID, {V, C}, PendingSlots),
		    % and tell the requesting process to continue
		    ?DEB("...and the original node continues.\n", []),
		    PID ! {continue},
		    wrun(ID, Ctxts, CID, WValues2, PendingSlots2);
		...
		{ok, [{pending, PID2}]} ->
		    ...
		    if  PID==PID2 ->
			    ...
			    PendingSlots2 = dict:append(PID2, PID, PendingSlots),
	...
	{regval, PID, Val} ->
		...
		{ok, PIDSlots} ->
		    ...
		    WValues2 = updateWH(PID, WValues, PIDSlots, Val),
		    wrun(ID, Ctxts, CID, WValues2, PendingSlots2)
	    end;
	{createCtxt, PID, J, C} ->
	...
	%%     PID ! {ctxtTail, CtxtTail},
	%%     wrun(ID, Ctxts, CID, WValues, PendingSlots);
	M -> io:fwrite("Warehouse received strange message: ~p\n", [M])
    end.
